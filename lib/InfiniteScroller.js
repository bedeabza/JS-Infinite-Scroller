(function () {
    ...
    var self = {
        ...
        init: function () {
            ...
            this.callbacks.becameInvisible(-1, this.inner.children[0]);
            for (i = 0;i < this.options.numElems - 2;i += 1) {
                this.callbacks.becameVisible(i, this.inner.children[i + 1]);
            }
            this.callbacks.becameInvisible(i, this.inner.children[i + 1]);
        ...
        executeEdgePass: function (index) {
            var state = this.state,
                dir = state.direction,
                rightIndex = index + this.options.numElems - 2,
                firstElem = this.inner.children[0],
                lastElem = this.inner.children[this.inner.childElementCount - 1],
                madeAnElementInvisible = false;

            this.log('passed edge');

            state.masterDirection = dir;
            ...
            if (state.progress === 0 || !this.passedEdgeLastTime) {
                this.callbacks.becameInvisible(dir === 1 ? rightIndex : index, dir === 1 ? lastElem : firstElem);
                madeAnElementInvisible = true;
            }

            if (state.progress !== 0) {
                this.callbacks.becameVisible(dir === 1 ? index - 1 : rightIndex + (madeAnElementInvisible ? 1 : 0), dir === 1 ? firstElem : lastElem);
            }
        },
        ...
        setNumElems: function (num) {
            if (this.options.numElems !== num) {
                this.executeWhenStopped(function () {
                    ...
                    if (this.options.numElems < num) {
                        //create new and set visibility
                        for (i = this.options.numElems - 1;i < num - 1;i += 1) {
                            this.callbacks.createElement(state.currentIndex + i, inner.children[i + 1]);
                            this.callbacks.becameVisible(state.currentIndex + i - 1, inner.children[i]);
                        }

                        //make last one invisible
                        this.callbacks.becameInvisible(state.currentIndex + num - 2, inner.children[inner.childElementCount - 1]);
                    } else {
                        //destroy the excess
                        for (i = this.options.numElems - 1;i > num - 1;i -= 1) {
                            this.callbacks.destroyElement(state.currentIndex + i - 1, inner.children[i]);
                        }

                        //marke last one invisible
                        this.callbacks.becameInvisible(state.currentIndex + num - 2, inner.children[i]);
                    }
