            ...
            if (newCount !== currentCount) {
                if (newCount > currentCount) {
                    for (i = currentCount;i < newCount;i += 1) {
                        ...
                        if (notify) {
                            index = this.state.currentIndex + i - 1;
                            elem = this.inner.children[this.inner.childElementCount - 1];

                            this.callbacks.createElement(index, elem);
                            this.callbacks.becameVisible(index, elem);
                        }
                    }
                } else {
                    for (i = currentCount;i > newCount;i -= 1) {
                        ...
                        if (notify) {
                            this.callbacks.destroyElement(this.state.currentIndex + i - 1, elem);
                        }

                        this.inner.removeChild(elem);
                    }
                }
            }
        },
