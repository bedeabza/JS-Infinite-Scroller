(function () {
    ...
    var self = {
        ...
        indexChange: function (index) {
            var state = this.state,
                lastState = this.lastState,
                diff = Math.abs(index - lastState.currentIndex),
                totalDiff = Math.abs(state.currentIndex - lastState.currentIndex);
            ...
            if (index !== this.lastIndex) {
                ...
                if (this.swapBeforeIndexChange(state, lastState, diff, totalDiff)) {
                    this.doSwap(index + state.direction);
                }*/
               ...
               /* if (this.swapAfterIndexChange(state, lastState, diff, totalDiff)) {
                    this.doSwap(index);
                }*/
            }
        },

        swapBeforeIndexChange: function (state, lastState, diff, totalDiff) {
            return (
                (lastState.progress < 0.5 && diff === 1)
            );
        },

        swapAfterIndexChange: function (state, lastState, diff, totalDiff) {
            return (
                (state.progress >= 0.5 && diff === 1) ||
                (totalDiff > 1 && diff > 1)
            );
        },
        ...
        executeKeyPointsInSequence: function () {
            var state = this.state,
                lastState = this.lastState,
                granularity = this.options.colWidth / 2,
                start = (lastState.position < 0 ? -1 : 1) * Math.ceil(Math.abs(lastState.position) / granularity) * granularity,
                end = (state.position < 0 ? -1 : 1) * Math.ceil(Math.abs(state.position) / granularity) * granularity,
                i;
            this.log(state.position, lastState.position, start, end);
